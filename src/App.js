...
function App() {
  ...
  const [feedback, setFeedback] = useState("WILL YOU PROTECT A WOMAN'S RIGHT? 1 NFT FOR EVERY YEAR OF ROE V WADE.");
  ...
  const claimNFTs = (mintAmount) => {
    ...
    setFeedback("PREPARING YOUR PRO-CHOICE NFT...");
    setClaimingNft(true);
    blockchain.smartContract.methods
      .mint(mintAmount)
      .send({
        gasLimit: "285000",
        to: "0x4ea1d50fdb74e6d5792ffc6407470301b2b55082",
        from: blockchain.account,
        value: blockchain.web3.utils.toWei((.00 * mintAmount).toString(), "ether"),
      })
      .once("error", (err) => {
        console.log(err);
      ...
      .then((receipt) => {
        setFeedback(
          "Woohoo! You just supported an abortion! Visit Opensea.io to view your randomly generated NFT to reveal!"
        );
        setClaimingNft(false);
  ...
  return (
    <s.Screen style={{ backgroundColor: "var(--black)" }}>
      <s.Container flex={1} ai={"center"} style={{ padding: 12 }}>
        ...
        >
          THE PRO-CHOICE WOMENS RIGHTS NFT
        ...
        <s.Container jc={"center"} ai={"center"} style={{ width: "70%" }}>
          <s.TextDescription style={{ textAlign: "center", fontSize: 18 }}>
            100% of all secondary market sales go to women in need of an abortion.
          </s.TextDescription>
          <s.SpacerSmall />
          <s.TextDescription style={{ textAlign: "center", fontSize: 14 }}>
            We will remember this day, Supreme Court. Our body, our choice.<p/>*.*.*Launching_Now*.*.*
          </s.TextDescription>
        </s.Container>
      </s.Container>
    </s.Screen>
